...
class RegionExtractor():
    ...
    def extract_regions(self, index):
        ...
        for i, sample in enumerate(self.samples[index]):
            regions[i] = self.crop_image2(self.image, sample, self.crop_size, self.padding)
        regions = regions.transpose(0, 3, 1, 2)
        regions = regions.astype('float32') - 128.
        return regions
    
    def crop_image2(self, img, bbox, img_size=107, padding=16, flip=False, rotate_limit=0, blur_limit=0):
