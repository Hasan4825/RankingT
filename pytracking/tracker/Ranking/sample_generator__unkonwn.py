...
from pytracking.tracker.Ranking.bbreg import overlap_ratio
...
class SampleGenerator():
    ...
    def _gen_samples(self, bb, n):
        ...
        samples[:, 2:] = np.clip(samples[:, 2:], 0.1, self.img_size - 1)
        if self.valid:
    ...
    def __call__(self, bbox, n, overlap_range=None, scale_range=None):
        ...
        if overlap_range is None and scale_range is None:
            ...
            while remain > 0 and factor < 16:
                ...
                for ind_ratio, ratio in enumerate(r):
                    if idx[ind_ratio]:
                        overlap_ratios_.append(ratio)  
                        
                samples_ = samples_[idx, :]
                samples_ = samples_[:min(remain, len(samples_))]
                if samples is None:
                    samples = samples_
                    overlap_ratios = overlap_ratios_
                else:
                    samples = np.concatenate([samples, samples_])
                    overlap_ratios = np.concatenate([overlap_ratios, overlap_ratios_])

                remain = n - len(samples)
                factor = factor * 2
 
            overlap_ratios = np.asarray(overlap_ratios)
            overlap_ratios = overlap_ratios[:n]
            return samples, overlap_ratios
