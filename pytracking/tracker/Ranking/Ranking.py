...
class Ranking(BaseTracker):
    ...
    def HardRanking(self, batch_rank_feats, batch_rank_ors, in_layer='fc4'):
        self.model.eval()
        ...
        self.model.train()
        return batch_rank_feats, batch_rank_ors
    ...
    def train(self, optimizer, pos_feats, neg_feats, rank_feats, rank_ors, maxiter, in_layer='fc4'):
     ...
     # hard negative mining
            ...
            if maxiter > 20 and i == 0: 
                self.params.hard_rank = False
            else :
                self.params.hard_rank  = True
            if self.params.hard_rank:       
                batch_rank_feats, batch_rank_ors = self.HardRanking(batch_rank_feats, batch_rank_ors)
            # forward
    ...
    def initialize(self, image, info: dict) -> dict:
        ...
        np.random.seed(0)
        torch.manual_seed(0)
        self.target_bbox = np.array(info['init_bbox'])
        ...
        rank_feats = self.forward_samples(image, rank_examples)

        self.train(self.init_optimizer, pos_feats, neg_feats, rank_feats, rank_ors, self.params.maxiter_init)
        
        self.bbreg_feats = self.forward_samples(image, bbreg_examples)
        self.bbreg = BBRegressor(image.size)
        self.bbreg.train(self.bbreg_feats, bbreg_examples, self.target_bbox)
   
       # Init sample generators for update
        self.sample_generator = SampleGenerator('gaussian', image.size, self.params.trans, self.params.scale, self.params.aspect_ratio)
        self.pos_generator = SampleGenerator('gaussian', image.size, self.params.trans_pos, self.params.scale_pos)
        self.neg_generator = SampleGenerator('uniform', image.size, self.params.trans_neg, self.params.scale_neg)
        self.rank_generator_up = SampleGenerator('gauussian', image.size, self.params.trans_rank_up, self.params.scale_rank_up)
        self.rank_generator_down = SampleGenerator('gauussian', image.size, self.params.trans_rank_down, self.params.scale_rank_down)
    ...
    def track(self, image, info: dict = None) -> dict:
     ...
     # Data collect
        if success:
            ...
            rank_examples = np.zeros((int(2*min(len(rank_examples_up),len(rank_examples_down))),4), dtype='float64')
            rank_ors = np.zeros((len(rank_examples),), dtype='float32')
            itr=0
            while itr<(len(rank_examples)/2):
                rank_examples[2*itr]=rank_examples_up[itr,:]
