...
@tensor_operation
def eca(input: torch.Tensor, weight: torch.Tensor):
    """Do a convolution with a 1x1 kernel weights. Implemented with matmul, which can be faster than using conv."""
    out = F.adaptive_avg_pool2d(input, 1)
    out = torch.conv1d(out.squeeze(-1).transpose(-1,-2), weight, padding=2).transpose(-1, -2).unsqueeze(-1)
    out = F.sigmoid(out)
