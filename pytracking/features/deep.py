...
class ATOMResNet18(MultiFeatureBase):
    ...
    args:
        ...
        use_gpu: Use GPU or CPU.
    """
    def __init__(self, clip_len=4, output_layers=('layer3',), net_path='atom_iou', use_gpu=True, *args, **kwargs):
    ...
    def initialize(self):
        ...
        self.layer_stride = {'conv1': 2, 'layer1': 4, 'layer2': 8, 'layer3': 16, 'layer3_2': 16, 'layer3_3': 16, 'layer4': 32, 'classification': 16, 'fc': None}
    ...
    def extract(self, im: torch.Tensor):
        ...
        if self.clip_len != 1:  
            if self.frame_num == 1:
                self.im = []
                ims = []
                self.im.extend(self.clip_len*[im[0].unsqueeze(0)])
                # self.im = [im[0].unsqueeze(0),im[0].unsqueeze(0),im[0].unsqueeze(0),im[0].unsqueeze(0)]           
                # ims = [im,im,im,im]
                ims.extend(self.clip_len*[im])
                im = torch.stack(ims, dim=2) 
            else:
                self.im.pop(0)
                self.im.append(im)
                im = torch.stack(self.im, dim=2) 
            self.frame_num += 1
        ...
        if len(self.iounet_backbone_features)>2: # both 2d and 3d
            cpy = self.iounet_backbone_features.copy()
            self.iounet_backbone_features[1] = cpy[2]
            self.iounet_backbone_features[2] = cpy[1]
