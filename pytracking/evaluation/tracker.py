...
class Tracker:
    ...
    def run_sequence(self, seq, visualization=None, debug=None, visdom_info=None, multiobj_mode=None):
        ...
        # inds = np.array(np.arange(0,100)//10)*0.01
        # params.size_change +=  inds[self.run_id]
        # params.aspect_ratio += (self.run_id%10)*0.01
    ...
    def _track_sequence(self, tracker, seq, init_info):
        ...
        # Initialize
        if self.name ==  'Ranking' or self.name ==  'RankingFlow' or self.name == 'RFAF' or self.name == 'MDNet':
            image = Image.open(seq.frames[0]).convert('RGB')
        else:
            image = self._read_image(seq.frames[0])
        
        if seq.dataset == 'vot':
            import pytracking.evaluation.vot2020 as vot
            init_info['init_mask'] = vot.make_full_size(init_info['init_mask'], (image.shape[1], image.shape[0]))
            init_info['init_bbox']= masks_to_bboxes(torch.from_numpy(init_info['init_mask']), fmt='t').squeeze().tolist()
    ...
    def run_webcam(self, debug=None, visdom_info=None):
        ...
        # cap = cv.VideoCapture('rtsp://192.168.43.1:8554/fpv_stream')
        ...
        while True:
            ...
            info = OrderedDict()
            info['previous_output'] = prev_output
            cv.rectangle(frame_disp, (int(640*0.25), int(480*0.10)), (int(640*0.75), int(480*0.9)),
                                     (180, 255, 255), 3)
            cv.putText(frame_disp, 'Av', (160, 450), cv.FONT_HERSHEY_COMPLEX_SMALL, 1, (255,0,0), 1)
            ...
            if ui_control.mode == 'select':
                ...
                cv.putText(frame_disp, 'Hedef IHA Takibi Basladi', (200, 30), cv.FONT_HERSHEY_COMPLEX_SMALL, 1, (255,0,255), 1)
                date1 = datetime.now()
            if len(sequence_object_ids) > 0:
                ...
                prev_output = OrderedDict(out)
                delta = date2 - date1
                cv.putText(frame_disp, 'Takip Suresi:' + str(delta.seconds), (10, 40), cv.FONT_HERSHEY_COMPLEX_SMALL, 1, (0,255,0), 1)
                if delta.seconds > 4.1:
                    cv.putText(frame_disp, 'Hedef Basariyla takip edildi', (200, 30), cv.FONT_HERSHEY_COMPLEX_SMALL, 1, (255,0,255), 1)
                    cv.waitKey(5)
                ...
                if 'target_bbox' in out:
                    for obj_id, state in out['target_bbox'].items():
                        ...
                        cv.rectangle(frame_disp, (state[0], state[1]), (state[2] + state[0], state[3] + state[1]),
                                     _tracker_disp_colors[8], 5)
                        cv.putText(frame_disp, 'Ah', (state[0], state[1]+state[3]+20), cv.FONT_HERSHEY_COMPLEX_SMALL, 1, (180,100,70), 1)



            # Put text
            # cv.putText(frame_disp, 'Select target UAV', (230, 15), cv.FONT_HERSHEY_COMPLEX_SMALL, 1, font_color, 1)
            # cv.putText(frame_disp, 'Press r to reset', (20, 55), cv.FONT_HERSHEY_COMPLEX_SMALL, 1,
            #            font_color, 1)
            ...
            cv.putText(frame_disp, 'Zaman:' + datetime.time(dateNow).strftime("%H:%M:%S"), (10, 15), cv.FONT_HERSHEY_COMPLEX_SMALL, 1, (0,255,0), 1)
            cv.imshow(display_name, frame_disp)
            key = cv.waitKey(1)
            if key == ord('q'):
                break
    ...
    def run_vot2020(self, debug=None, visdom_info=None):
        ...
        if params.segmentation is True:
    ...
    def init_visualization(self):
        ...
        self.fig.text(10, 10, 'Starting Tracking', fontsize= 50)
        plt.tight_layout()
