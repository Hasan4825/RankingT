...
class VOTDataset2021(BaseDataset):
    """
    VOT2018 dataset
    ...
    def _construct_sequence(self, sequence_name):
        ...
        for frame_num in range(end_frame):
            gt_seg = trax.Mask.create(gt_all[frame_num].mask, gt_all[frame_num].offset[0], gt_all[frame_num].offset[1])
        ...
        return Sequence(sequence_name, frames, 'vot', ground_truth_rect, gt_seg_all)

    def __len__(self):
        return len(self.sequence_list)

    def _get_sequence_list(self):
        sequence_list = np.loadtxt('{}/list.txt'.format(self.base_path), dtype=str).tolist()
