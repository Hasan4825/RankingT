...
class Display:
    ...
    def show(self, image, bb_list, trackers, gt=None):
        ...
        for i, bb in enumerate(bb_list):
            rect = patches.Rectangle((bb[0], bb[1]), bb[2], bb[3], linewidth=2,
        ...
        self.ax.axis('equal')
        plt.savefig('/run/media/hasan/DATA/Hasan/pytracking/pytracking/plots/'  + '%s' %self.sequence_name + '/' + '%d' %self.frame_number + '.png')        
...
def playback_results(trackers, sequence):
    ...
    tracker_results = []
    name_old = sequence.name
    # Load results
    for trk_id, trk in enumerate(trackers):
        # Load results
        sequence.name = name_old
        # if trk.name == 'atom' or trk.name == 'ECO' or trk.name == 'MDNet':
        #     sequence.name = 'nfs_' + sequence.name
        # else:
...
def main():
    trackers = []
    trackers.extend(trackerlist('dimpRank', 'dimpRank18', range(0,1), 'dimpRank'))
