...
# -*- coding: utf-8 -*-
"""
Created on Sat Apr 10 13:15:04 2021
...
def plot_draw_save(y, x, scores, trackers, plot_draw_styles, result_plot_path, plot_opts):
    ...
    font_size_axis = plot_opts.get('font_size_axis', 40)
    line_width = plot_opts.get('line_width', 7)
...
def plot_results(trackers, dataset, report_name, merge_results=False,
    ...
    if 'success' in plot_types:
        ...
        success_plot_opts = {'plot_type': 'success', 'legend_loc': 'lower left', 'xlabel': 'Overlap threshold',
                             'ylabel': 'Success rate', 'xlim': (0, 1.0), 'ylim': (0, 93), 'title': 'Success plots of OPE'}
        # tracker_names.append({'name': 'New', 'param': 'default', 'run_id': 0, 'disp_name': 'New'})
        # auc_curve = torch.cat((auc_curve, 0.5*auc_curve[0].reshape(1,-1)), 0)
        # auc = torch.cat((auc, 0.5*auc[0].reshape(1)), 0)
    ...
    if 'prec' in plot_types:
        ...
        precision_plot_opts = {'plot_type': 'precision', 'legend_loc': 'lower right',
                               ...
                               'xlim': (0, 50), 'ylim': (0, 95), 'title': 'Precision plots of OPE'}
        # tracker_names.append({'name': 'New', 'param': 'default', 'run_id': 0, 'disp_name': 'New'})
        # prec_curve = torch.cat((prec_curve, 0.5*prec_curve[0].reshape(1,-1)), 0)
        # prec_score = torch.cat((prec_score, 0.5*prec_score[0].reshape(1)), 0)
    ...
    if 'norm_prec' in plot_types:
        ...
        norm_precision_plot_opts = {'plot_type': 'norm_precision', 'legend_loc': 'lower right',
                                    ...
                                    'xlim': (0, 0.5), 'ylim': (0, 80), 'title': 'Normalized Precision plot'}
