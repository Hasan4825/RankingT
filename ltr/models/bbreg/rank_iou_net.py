...
class RankIoUNet(nn.Module):
    ...
    def get_modulation(self, feat, bb):
        ...
        # examples, ious = self.Sample_select((288,288), bb[0])
        feat3_r, feat4_r = feat
        c3_r = self.conv3_1r(feat3_r)
        c4_r = self.conv4_1r(feat4_r)
        # Add batch_index to rois
        batch_size = bb.shape[0]
        # batch_index = torch.arange(batch_size, dtype=torch.int).reshape(-1, 1).to(bb.device)
        batch_index = torch.tensor(np.array(range(0,batch_size))//500, dtype=torch.float32).reshape(-1, 1).to(feat3_r.device) 
        # input bb is in format xywh, convert it to x0y0x1y1 format
        # bb = bb.clone()
        # bb[:, 2:4] = bb[:, 0:2] + bb[:, 2:4]
        # examples[:, 2:4] = examples[:, 0:2] + examples[:, 2:4]
